...
# gitops/platform/argocd.yaml, the self-management Application (R22) that
# takes over managing this same Helm release once it syncs.
#
# VALUES-SYNC CONTRACT (R17 <-> R22) — read before touching either side:
# local.argocd_helm_values below MUST stay identical to the valuesObject in
# gitops/platform/argocd.yaml, using the same chart version pinned here
# (10.1.1). ArgoCD's first sync of that self-management Application diffs
...
# R18/R21: the single bootstrap Application (the app-of-apps root). Applied
# via the kubectl provider (see argocd-providers.tf) so `plan` doesn't need
...
# (chicken-and-egg). Points at gitops/bootstrap, a directory of child
# Applications (gitops/bootstrap/platform.yaml, gitops/bootstrap/workloads.yaml)
# that in turn reconcile gitops/platform/ and gitops/workloads/ — so adding a
# platform add-on or a workload later is a commit under gitops/, never a
# second `terraform apply` (R21).

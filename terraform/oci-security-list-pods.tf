# R10: pods security list. The OKE cluster's CNI is OCI_VCN_IP_NATIVE, so
# pods draw IPs from this subnet directly and need their own security list
# distinct from worker nodes.
#
# Deviation from the reference: the reference's "Allow pods to communicate
# with Plex" egress rule (TCP 32400) is source-specific homelab cruft and is
# dropped here — every generic OKE rule is kept.
